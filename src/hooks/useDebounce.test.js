...
import useDebounceUndo from './useDebouncedUndo'

test('should use debounced undo', () => {
    const { result } = renderHook(() => useDebounceUndo())
...
test('should update content immedialtely', () => {
    const { result } = renderHook(() => useDebounceUndo())
...
test('should debounce undo history update', async () => {
    const { result, waitForNextUpdate } = renderHook(() => useDebounceUndo())
